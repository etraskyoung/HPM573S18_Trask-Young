...
def add_background_mortality(prob_matrix):
    ...
    for s in HealthStats:
        ...
        if s == [HealthStats.WELL]:
            rate_matrix[s.value][HealthStats.BACKGROUND_DEATH.value] = -np.log(vData.ANNUAL_PROB_BACKGROUND_MORT)
            rate_matrix[s.value][HealthStats.SROKE_DEATH.value] =  -np.log(1 - Data.ANNUAL_PROB_BACKGROUND_MORT)
